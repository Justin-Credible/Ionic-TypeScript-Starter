...
The init task is used to initialize the Cordova project by adding platforms, plugins, obtaining third party libraries, and performing TypeScript compilation.
...
The config task is used to perform customization of configuration files based on the given build scheme name. Build schemes are defined in `config.master.xml` under the `schemes` node. If a scheme name is not specified, the default will be used.
...
These files will be generated from their master files and variable substitution will be performed based on the scheme definitions. This allows for easy configuration of builds for different environments (ie development, staging, production).
...
Initially, Bower will download the files to the `bower_components` directory and the `bower-installer` plugin will take care of copying the appropriate files to the `www/lib` directory.

See `bower.json` for more details (as well as for if you need to change which files get copied to the libs directory).
...
The tsd task is used to download and setup TypeScript definition files for JavaScript libraries so they can be referenced from TypeScript source files. The `tsd` tool uses the [DefinitelyTyped](http://definitelytyped.org/) repository.
...
The chrome task is used to generate a `chrome` directory with your application that can be loaded as a Chrome extension.
...
!!! note
	If you are using VS Code, you can use <kbd>⌘ Command</kbd><kbd>⇧ Shift</kbd> <kbd>R</kbd> and being typing the name of the task your want to run. See `.vscode/tasks.config` for more task shortcuts.
...
This task is a shortcut for `cordova emulate ios` which is a useful shortcut to use from VS Code's task runner.

## gulp emulate-android

This task is a shortcut for `cordova emulate android` which is a useful shortcut to use from VS Code's task runner.

## gulp remote-emulate-ios

This task allows Windows or Linux developers to run the iOS simulator on a remote Mac OS X computer.
...
After cleaning, you may need to re-run `gulp init` to ensure your environment is re-initialized.
