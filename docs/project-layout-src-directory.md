...
The `src` directory contains the bulk of the source code for the application. This includes TypeScript source, SASS styling, and Angular HTML templates.
...
In most example Angular applications you'll commonly see seperate directories for each component (eg controllers, views, models, services etc). While this is nice for tutorials and small applications, it is not ideal for applications that contain a large number of files.
...
The main application class. It's `start()` method is invoked by the second level boot loader (`src/Framework/Boot2.ts`).

Here you can subscribe to device events (eg pause, resume) or configure Cordova plugins when the application starts.
...
This file contains a static namespace housing constants for use throughout the application.

## Application/RouteConfig.ts

This file contains a single method named `setupRoutes(...)` which is delegated to from the second level boot loader (`src/Framework/Boot2.ts`).

It is responsible for setting up all of the Angular routes by defining controller and view to use for each route.

# Framework

The framework directory contains components that make up the "framework" for the application. This includes base classes as well as the second level boot loader.
...
This contains helper methods for use by `src/Framework/Boot2.ts` such as controller or service registration etc.

## Framework/BaseController.ts

This is a base class that should be used by all of the Angular controllers.

See [Base Framework](base-framework.md) for more information.
...
This is where you can store model objects that are not view models or are models that are shared between multiple areas or services in the application.

# Services

This is where all of your [Angular services](https://docs.angularjs.org/guide/services) sholud live.

There are several services provided with the sample project; see [Base Framework](base-framework.md) for more details.

# Styles

This is where you should store your common or shares sass styles. The `Index.scss` file is used to reference all of the sass files to be compiled.
...
The default layout uses a directory for each view which has its controller, view model, template and styling in the same directory.

Each controller should extend `BaseController` or `BaseDialogController`.

See `gulp templates` and `gulp sass` in [Gulp Tasks](gulp-tasks.md) and [Base Framework](base-framework.md) for more details.